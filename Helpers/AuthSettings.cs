namespace user_management.Helpers
{
    public static class AuthSettings
    {
        // Secret key used for signing tokens.
        public static string PrivateKey { get; set; } = "";

        // Token issuer.
        public static string Issuer { get; set; } = "only_myanmar";

        // Token audience.
        public static string Audience { get; set; } = "user";

        // Token expiration time.
        public static TimeSpan TokenExpiration { get; set; } = TimeSpan.FromHours(1);
    }
}